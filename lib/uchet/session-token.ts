...
export function createUchetSessionToken(): string | null {
  ...
  const payload: SessionPayload = {
    sub: "uchet",
    iat: now,
    exp: now + UCHET_SESSION_MAX_AGE * 1000,
  };
  const data = base64UrlEncode(JSON.stringify(payload));
  return `${data}.${signData(data, secret)}`;
}

export function verifyUchetSessionToken(token: string): boolean {
  const secret = getUchetSessionSecret();
  ...
  try {
    ...
    return payload.sub === "uchet" && payload.exp > Date.now();
  } catch {
    return false;
  }
}
