...
type ParserErrorStateProps = {
  message: string;
  onRetry?: () => void;
};

export function ParserErrorState({ message, onRetry }: ParserErrorStateProps) {
  return (
    <motion.div
    ...
    >
      <GlassCard className="p-10 sm:p-12 text-center" hover={false}>
        ...
        <p className="mx-auto mt-2 max-w-md text-sm text-red-600/90">{message}</p>
        {onRetry && (
          <button
            type="button"
            onClick={onRetry}
